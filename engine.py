...
class Engine:
    ...
    def __init__(self, config: dict | None = None):
        ...
        # wire features → bridge
        self.bridge.infinite_stamina    = bool(cfg.get("stamina", False))
        self.bridge.defense_enabled     = bool(cfg.get("defense", True))

        # wire detector → bridge fire
        tempo_ms = int(cfg.get("tempo", 39))
        tempo_on = bool(cfg.get("tempo_on", False))
        self.bridge.stick_tempo_enabled = tempo_on
        self.detector.set_release_callback(
            lambda l2=False: self.bridge.fire_shot(
                l2=l2, tempo=tempo_on, tempo_ms=tempo_ms)
        )

        self.capture_gpu = int(cfg.get("pc_mode", 0))
